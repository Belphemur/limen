...
func NewHandler(rt *boot.Runtime, resolver session.Resolver, bearerIntercept connect.UnaryInterceptorFunc) (string, http.Handler) {
	svc := session.NewService(resolver, bearerIntercept, rt.Logger)
	return svc.Handler()
}
