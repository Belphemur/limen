...
// apps may be nil in tests that don't exercise the MCP-client RPCs.
func NewService(store *storage.Store, upstreamSvc *upstream.Service, apps AppManager, resolver session.Resolver, bearerIntercept connect.UnaryInterceptorFunc, logger *zap.Logger) *Service {
	if logger == nil {
		logger = zap.NewNop()
	}
	return &Service{store: store, upstream: upstreamSvc, apps: apps, resolver: resolver, bearerIntercept: bearerIntercept, logger: logger}
}
