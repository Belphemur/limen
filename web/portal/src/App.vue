...
// Full navigation across SPAs — admin lives at /t/<tenant>/admin/.
const adminHref = computed(() => `${tenantPrefix() ?? ''}/admin/`)
  ...
  <div class="flex min-h-screen flex-col bg-bg-main text-on-surface">
    ...
    >
      <div class="mx-auto flex h-portal-header max-w-6xl items-center justify-between px-4">
        <RouterLink
          to="/"
          class="flex items-center gap-3 font-display text-xl font-semibold tracking-tight"
        >
          <img
            :src="logoUrl"
            alt=""
        ...
        <nav v-if="authenticated" class="flex gap-1 text-sm font-medium">
          ...
          <RouterLink
            ...
            class="rounded-md px-3 py-1.5 text-on-surface-variant transition-colors hover:bg-surface-container-low hover:text-on-surface"
            :class="
              route.path.startsWith('/mcp-clients')
                ? 'bg-surface-container-low text-on-surface'
                : ''
            "
          >
          ...
          <RouterLink
            ...
            class="rounded-md px-3 py-1.5 text-on-surface-variant transition-colors hover:bg-surface-container-low hover:text-on-surface"
            :class="
              route.path.startsWith('/settings') ? 'bg-surface-container-low text-on-surface' : ''
            "
          >
            Settings
          </RouterLink>
