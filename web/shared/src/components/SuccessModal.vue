...
<template>
  <BaseModal :open="open" testid="success-modal" @close="emit('close')">
    <div
      ...
      <div class="relative mb-6">
        <div class="absolute inset-0 animate-ping rounded-full bg-success/10" style="animation-duration: 2s"></div>
        <div class="relative z-10 flex h-24 w-24 items-center justify-center rounded-full bg-success/20">
          <CheckCircle2 :size="56" class="text-success" aria-hidden="true" />
        </div>
      </div>
      ...
      <div v-if="chip"
        ...
        <span class="font-data-mono text-data-mono font-medium tracking-wide text-primary">
          {{ chip }}
        </span>
      </div>
      <p class="mb-10 max-w-[400px] font-body-lg text-body-lg text-on-surface-variant">
        {{ message }}
      </p>
      <div class="flex w-full flex-col gap-4 sm:flex-row">
        <button v-if="secondaryLabel" type="button"
          ...
          data-testid="success-modal-secondary" @click="emit('secondary')">
          {{ secondaryLabel }}
        </button>
        <button v-if="primaryLabel" type="button"
          ...
          data-testid="success-modal-primary" @click="emit('primary')">
          {{ primaryLabel }}
        </button>
      </div>
    </div>
