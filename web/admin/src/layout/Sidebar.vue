...
<template>
    <aside
        ...
        <div class="flex items-center gap-3 px-6 py-5">
            <img :src="logoUrl" alt="" aria-hidden="true" width="32" height="32"
        ...
        <nav class="flex flex-1 flex-col gap-1 overflow-y-auto px-3">
            <template v-for="node in navTree" :key="node.label">
                <RouterLink v-if="node.kind === 'leaf'" :to="node.path"
                    class="flex items-center gap-3 rounded-lg px-4 py-3 transition-colors" :class="isActive(node.path)
        ...
        <div class="mt-auto border-t border-sidebar-divider p-3">
            <RouterLink :to="ROUTES.mcpServerNew"
            ...
            <a href="https://limen.dev/docs/support" target="_blank" rel="noreferrer noopener"
