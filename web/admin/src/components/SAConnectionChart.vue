  ...
  try {
    ...
    if (resp.hasData && resp.days.length > 0) {
      chartData.value = {
        ...
        datasets: [
          {
            label: 'Peak SA Connections',
            data: resp.days.map((d) => d.peakConnections),
            borderColor: 'var(--color-tertiary)',
            // Canvas fill uses hard-coded rgba because CSS variables cannot carry alpha in Chart.js.
            backgroundColor: 'rgba(153, 60, 0, 0.1)',
            fill: true,
            tension: 0.3,
            pointRadius: 2,
            pointHoverRadius: 4,
...
<template>
  <div class="rounded-xl border border-outline-variant bg-surface-container-lowest p-5">
    <h3 class="text-sm font-semibold text-on-surface">SA Connections</h3>
    <p class="mt-0.5 text-xs text-on-surface-variant">
      Peak concurrent service account connections per day
    </p>
