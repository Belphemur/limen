  ...
  <div class="space-y-4">
    <div>
      ...
      <input
        ...
        required
        :minlength="complexity?.minLength ?? 8"
        :disabled="disabled"
    ...
    <div
      v-if="complexity"
      class="space-y-1.5 rounded-md bg-surface-2 p-3.5 text-body-xs border border-divider"
    >
      ...
      <ul class="space-y-1 mt-1.5">
        <li v-for="r in rules" :key="r.label" class="flex items-center gap-2">
          <span :class="r.valid ? 'text-success' : 'text-text-muted'">
            <svg
              v-if="r.valid"
              class="h-4 w-4 shrink-0 fill-current text-success"
              viewBox="0 0 20 20"
            >
              <path
                fill-rule="evenodd"
          ...
          <span :class="r.valid ? 'text-success font-medium' : 'text-text-muted'">{{
            r.label
          }}</span>
        </li>
      </ul>
