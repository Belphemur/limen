...
import { ErrorModal, SuccessModal, fetchDiscovery, zitadelConsoleUrl } from '@limen/shared'
...
const consoleUrl = computed(() =>
  loaded.value ? zitadelConsoleUrl(issuer.value, loaded.value.zitadelOrgId, 'users') : '',
)
...
onMounted(async () => {
  try {
    ...
    hydrate(settings.settings?.name ?? '', settings.settings?.publicId ?? '', settings.zitadelOrgId)
  } catch (err) {
  ...
  <div class="space-y-stack-lg">
    <header>
      ...
      <p class="mt-2 max-w-2xl text-sm text-on-surface-variant">
        Configure your organization name and cross-reference your Zitadel identity. The IDE
        redirect-URI allowlist lives on its own page under Organization → IDE Configuration.
        Destructive actions live in the Danger Zone at the bottom.
      </p>
    </header>
    ...
    <template v-else-if="loaded">
      ...
      >
        <h2 id="org-heading" class="text-lg font-semibold text-on-surface">Organization</h2>
        <label for="org-name" class="block text-sm font-medium text-on-surface"> Name </label>
        <input
          id="org-name"
          v-model="nameDraft"
          type="text"
        ...
        <div>
          ...
          <div class="mt-1 flex items-center gap-2">
            <code
              class="rounded bg-surface-variant px-2 py-1 font-mono text-sm text-on-surface"
              data-testid="org-public-id"
              >{{ loaded.publicId }}</code
            >
            <button
              type="button"
              ...
              data-testid="org-public-id-copy"
              @click="copyPublicId"
            >
              <Copy class="h-4 w-4" />
            </button>
      ...
      <!-- 2. Zitadel identity -->
      <section
        aria-labelledby="zitadel-heading"
        class="space-y-3 rounded-lg border border-outline-variant bg-surface p-6"
        data-testid="section-zitadel"
      >
        <h2 id="zitadel-heading" class="text-lg font-semibold text-on-surface">Zitadel identity</h2>
        <p class="text-sm text-on-surface-variant">
          Limen mirrors your organization 1:1 against a Zitadel org. Users, roles, and SSO
          configuration are owned by Zitadel.
        </p>
        <div>
          ...
          <code
            ...
            data-testid="zitadel-org-id"
            >{{ loaded.zitadelOrgId }}</code
          >
        </div>
        <a
          v-if="consoleUrl"
        ...
        <p v-else class="text-sm text-on-surface-variant">
          Zitadel Console link is unavailable — the deployment is missing an issuer in
          /auth/discovery.
        </p>
      </section>
      ...
      <section
        ...
        class="space-y-3 rounded-lg border border-error/40 bg-error/5 p-6"
      ...
      >
        ...
        <p class="text-sm text-on-surface-variant">
          Deleting the organization soft-deletes every upstream, user binding, and per-user link
          Limen knows about. The Zitadel org is left untouched — its lifecycle is owned outside
          Limen.
        </p>
        <button
          type="button"
