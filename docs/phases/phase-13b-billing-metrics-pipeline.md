...
- [x] Dead-letter stream `billing:dlq` for entries with delivery count ≥ 5 (MAXLEN 10000)
- [x] All-in-one fallback: drain goroutine when `valkey.enabled: false`
...
- [x] Prometheus metrics: `limen_billing_events_dropped_total`, `limen_billing_stream_evicted_total`
- [x] Unit tests: recorder under load, channel backpressure, shed-load
- [x] Integration tests: RLS isolation, month-boundary, SA concurrent-count correctness
- [x] Integration tests: Valkey Stream consumer (happy path, crash recovery, dead-letter)
...
- **SSE disconnect**: Tracked via the `r.Context().Done()` pattern in `internal/transport/codemode_server.go` SSEHandler — a goroutine fires `RecordSAConnection(connected: false)` when the client disconnects, gracefully or abruptly. Stale rows from process crashes (no graceful disconnect) still inflate `concurrent_count` until a future periodic reconciliation sweeper is added.
