...
Limen ships as **six separate Go binaries**, each with a distinct role, threat model, and secret footprint:
...
| `limen-observer` | Billing & telemetry consumer | No (internal background worker) | DB app DSN, token encryption key, Valkey password. **MUST NOT** hold Zitadel admin credential or portal session cipher key |
...
The production binaries (`limen-gateway`, `limen-portal`, `limen-staff`, `limen-observer`) accept `--config` or `LIMEN_CONFIG` env var and parse flags via the stdlib `flag` package. `limenctl` uses Cobra for its subcommands and also accepts `--config` / `LIMEN_CONFIG`.
...
2. **Independent scaling** — gateway replicas can be scaled horizontally without also spinning up the heavier portal processes; portal, staff, and observer can scale separately based on their own traffic profiles.
...
services:
  ...
  limen-observer:
    image: ghcr.io/belphemur/limen-observer:${LIMEN_VERSION}
    restart: unless-stopped
    command: ["limen-observer", "--config", "/etc/limen/config.yaml"]
    environment:
      LIMEN_CONFIG: /etc/limen/config.yaml
      LIMEN_DB_DSN_FILE: /run/secrets/limen_db_app_dsn
      LIMEN_TOKEN_ENCRYPTION_KEY_FILE: /run/secrets/limen_token_encryption_key
...
| Secret | Gateway | Portal | Staff | Observer | limenctl (migrate) |
|--------|---------|--------|-------|----------|---------------------|
| `limen_db_app_dsn` | Yes | Yes | Yes | Yes | No |
| `limen_db_owner_dsn` | No | Yes (read) | No | No | Yes |
| `limen_token_encryption_key` | Yes | Yes | No | Yes | Yes |
| `limen_oidc_portal_client_id` | No | Yes | No | No | No |
| `limen_oidc_mgmt_pat` | No | Yes | Yes | No | No |
| `limen_staff_zitadel_org_id` | No | No | No | No | Yes |
| `limen_valkey_password` | Yes | Yes | No | Yes | No |
...
| `observer.Dockerfile` | `ghcr.io/belphemur/limen-observer` |
...
CI runs a matrix build across all six Dockerfiles on every PR. On tag push, all six images are built and pushed to GHCR.

## Migration strategy

- Schema migration runs as a **one-shot service** (`limenctl-migrate`) with `restart: "no"` and `condition: service_completed_successfully` gating the long-running `limen-gateway`, `limen-portal`, `limen-staff`, and `limen-observer` services. This ensures the schema is up-to-date before traffic flows.
...
- Rolling deploys: `limenctl-migrate` is run with the new image version _before_ the `limen-gateway`, `limen-portal`, `limen-staff`, and `limen-observer` services are updated, manually or via the deploy script.
...
```
postgres ──healthy──▶ zitadel-api ──healthy──▶ zitadel-bootstrap ──completed──▶ limenctl-migrate ──completed──▶ limen-gateway / limen-portal / limen-staff / limen-observer
```
...
```yaml
  limen:
    ...
    environment:
      LIMEN_CONFIG: /etc/limen/config.yaml
      # ... all secrets (gateway + portal + staff + observer combined)
...
- Logs are tagged with the binary name via the container name in compose (`limen-gateway`, `limen-portal`, `limen-staff`, `limen-observer`), making log filtering straightforward.
...
- Restore procedure documented in `docs/runbook.md`:
  1. Stop `limen-gateway`, `limen-portal`, `limen-staff`, and `limen-observer` (Zitadel continues running).
...
- `build/docker/gateway.Dockerfile`, `build/docker/portal.Dockerfile`, `build/docker/staff.Dockerfile`, `build/docker/observer.Dockerfile`, `build/docker/limenctl.Dockerfile`, `build/docker/limen.Dockerfile`
...
- **Secret sprawl**: secrets across five binaries means more files to manage than a single-container deploy. Automation (e.g., SOPS, Vault) is recommended for teams running this at scale.
...
- [ ] `compose.prod.yaml` defines `caddy`, `postgres`, `postgres-zitadel`, `valkey`, `limenctl-migrate`, `zitadel-bootstrap`, `limen-gateway`, `limen-portal`, `limen-staff`, `limen-observer`, `zitadel-api`, `zitadel-login`, `backup`, `backup-zitadel`
...
- [ ] `limenctl-migrate` runs as a one-shot, gates `limen-gateway` + `limen-portal` + `limen-staff` + `limen-observer` via `condition: service_completed_successfully`
...
- [ ] Per-binary Dockerfiles under `build/docker/` for gateway, portal, staff, observer, limenctl, all-in-one
- [ ] CI matrix builds all six images on every PR, pushes on tag
