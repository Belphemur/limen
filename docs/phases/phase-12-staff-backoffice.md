...
This phase intentionally sits **after** the customer-facing portal (Phase 9) and hardening (Phase 10), and **after** production deployment (Phase 11) only on paper — in practice the staff bootstrap step must run as part of the first deploy. Phase 11's `limen-migrate` is extended here to provision the staff tenant idempotently.
...
5. SPA at `/t/<target-slug>/portal/` detects `impersonation=true` (from the bootstrap call's response) and renders a **persistent red banner**: *"You are viewing **`alice@example.com`** on behalf of **`staff@limen.dev`**. Reason: `<reason>`. Expires in `09:42`. [End impersonation]"*. The banner cannot be dismissed; it pins to the viewport.
...
- Encrypted upstream tokens (`upstream_links.access_token_ciphertext` etc.) remain encrypted regardless of session. The impersonated session can *see* the link state (connected / disabled / auto-disabled / needs-relink) and *trigger user-facing actions* (e.g. start a reconnect flow), but Limen never decrypts the token "for staff" — only the upstream-MCP transport (running under the target's request) ever decrypts, at call time, exactly as if the customer were driving.
