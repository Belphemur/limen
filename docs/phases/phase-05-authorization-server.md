---
phase: "5"
title: "Zitadel integration (AS delegation + DCR proxy)"
status: in_progress
progress: 92
depends_on: ["4"]
updated: "2026-03-15"
...
- Portal admin RPCs ([Phase 9b](phase-09b-portal-spa.md)) — invite / disable / role-change / MCP-client revocation.
...
- [x] `internal/oauthproxy/uripolicy.go` implements the shared floor + allowlist matcher; reused by the DCR proxy and the [Phase 9c](phase-09c-tenant-admin-spa.md) tenant-admin validation RPC
...
- [ ] Allowlist surfaced in the [Phase 9c](phase-09c-tenant-admin-spa.md) tenant-admin SPA Settings page (gated by `RequireRole(owner|admin)`); `limen create-tenant` accepts a repeatable `--dcr-redirect-uri-allow` flag for operator bootstrapping
