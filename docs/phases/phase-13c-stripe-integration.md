...
- [x] Makefile target: `make stripe-bootstrap` runs `go run ./scripts/stripe-bootstrap/`

### 13c-b: Backend Integration

- [x] Add `github.com/stripe/stripe-go/v82` to `go.mod`
- [x] `tenant_billing` Goose migration with RLS + partial unique + staff-mode SELECT clause
- [x] `tenant_entitlements` Goose migration with RLS + (tenant_id, feature) unique + index
- [x] `internal/billing/entitlements.go` — `PlanEntitlements` struct + `EntitlementsFromRows()` resolver
- [x] `BillingConfig` struct in `internal/config/config.go`
- [x] `billing:` section in `config.yaml` with Stripe keys, price IDs, trial_days, grace_days
- [x] `internal/billing/stripe/client.go` — Stripe SDK wrapper with resilience
- [x] Customer lookup/create by tenant ID with `tenant_public_id` metadata
- [x] Checkout session creation with both line items (active_user + sa_connection prices)
- [x] Customer Portal session creation
- [x] `internal/billing/stripe/webhook.go` — handler at `/billing/stripe/webhook`
- [x] Signature verification via `stripe.Webhook.ConstructEvent`
- [x] Async drain: enqueue to in-memory channel, ACK immediately, drain goroutine mutates Postgres
- [x] Event: `checkout.session.completed` — persist sub ID + price IDs, flip plan/status, run Reconcile
- [x] Event: `customer.subscription.updated` — mirror status, period_end, cancel_at_period_end, prices
- [x] Event: `customer.subscription.deleted` — set status='canceled', plan='developer', clear sub ID
- [x] Event: `invoice.payment_failed` — set grace_until = now + grace_days
- [x] Event: `invoice.payment_succeeded` — clear grace_until
- [x] Event: `entitlements.active_entitlement_summary.updated` — parse lookup_keys, DELETE + UPSERT `tenant_entitlements`, derive `plan`
- [x] `BillingService` proto in `proto/limen/portal/v1/portal.proto` + `buf generate`
- [x] `GetBillingSummary` RPC: plan, status, active counts, publishable key
- [x] `CreateCheckoutSession` RPC: Stripe Checkout with line items from billing metrics
- [x] `OpenCustomerPortal` RPC: Stripe Customer Portal session
- [x] `internal/billing/reconciler.go` — 1h jittered periodic loop
- [x] Reconciler: reactive hook on billing metrics events
- [x] Reconciler: upward-only quantities within billing month
- [x] Reconciler: month-boundary reset to new month's count
- [x] Startup reconciliation: List Active Entitlements API for active tenants
- [x] Resilience config for Stripe endpoints in `config.yaml` resilience section
